...
### Bug Fix — Goalkeeper save% calculation + scraper self-heal for partial stats

- **Completed:** 2026-07-12
- **Files changed:** `src/lib/data.js` (updated), `src/components/GoalkeeperTable.svelte` (updated), `scripts/scrape.js` (updated)
- **What was done:** Corrected the goalkeeper save% calculation and made the scraper resilient to incomplete stat feeds.
  - **Save%:** `loadGoalkeeperStats` computed save% as total saves ÷ a back-inferred denominator, using `GoalkeeperSaves` (all saves) against a save% that is defined on on-target shots only — a category error that systematically understated save% (e.g. Diogo Costa 86.2% → 95.0%, Schlager 57.7% → 100%). Fixed to the standard FBref/Opta definition: shots-on-target faced = `GoalkeeperSavesOnTarget ÷ GoalkeeperSavePercentage` per match (back-inferred because the feed omits `GoalsConceded` in ~half of matches), then aggregate save% = Σ on-target saves ÷ Σ shots faced. Also repointed the table's "SoT Faced" column from `GoalkeeperSavesOnTarget` (on-target _saves_) to the derived `shotsOnTargetFaced`.
  - **Scraper:** `shouldFetch` only checked file existence and the manifest permanently marked matches done, so a `player-stats.json` captured before FIFA finalized the feed (e.g. match 400021478, truncated to ~61 of 112 fields for all players) was cached forever. Added file-level completeness detection (`recordsComplete`: a finalized match has ≥1 full ~112-field record) and a bounded self-heal — incomplete files are re-fetched on subsequent runs, capped at `MAX_PLAYER_STATS_RETRIES` (5) via `manifest.playerStatsRetries`, and a complete file is never overwritten by an incomplete refetch.
- **Verification:** Cross-checked against Fox Sports and official FIFA.com stats. Our totals match FIFA exactly for every keeper whose matches were fully scraped (Room, Costa, Bounou, Vozinha, Suzuki, Alowais, Nyland, Beiranvand). Save% reconciles with Fox goals-against to ±1 (penalties/own goals). `node scripts/scrape.js --selfcheck` passes; `npm run build` succeeds (110 pages). Dry-run confirms the self-heal flags exactly the one truncated match (400021478).
- **Known limitation:** FIFA's per-match stats API (`fdh-api.fifa.com`) _permanently_ omits some data — match 400021478 (all players truncated) and keepers Gill/Kobel/Courtois/Verbruggen in matches 400021522/533/535 (keeper absent while teammates are complete). Re-fetching does not recover it (verified live); FIFA.com's official figures come from a separate internal pipeline (`competitionId 285023`). So those 4 keepers undercount vs FIFA until/unless the upstream feed is backfilled. The self-heal will pick it up automatically if that happens.

### Feature — Opta-sourced goalkeeper page (separate from FIFA page)

- **Completed:** 2026-07-13
- **Files changed:** `scripts/scrape-goalkeepers.js` (new), `scraped-data/goalkeepers.json` (new, generated), `src/lib/data.js` (added `loadGoalkeeperStatsOpta`), `src/components/GoalkeeperTableOpta.svelte` (new), `src/pages/goalkeepers-opta.astro` (new), `src/layouts/Base.astro` (nav link), `package.json` (`scrape:gk` script)
- **What was done:** Added a second goalkeeper page sourced entirely from the Thomson Reuters/Opta feed, leaving the existing FIFA-sourced `/goalkeepers` page untouched.
  - **Scraper:** `scripts/scrape-goalkeepers.js` pulls the Opta `matches.json`, filters to played matches, fetches each `match-stats/{id}.json`, keeps `position: "Goalkeeper"` players, and aggregates a tournament-wide table into `scraped-data/goalkeepers.json` (57 keepers from 100 matches). Has a `--selfcheck`. Run via `npm run scrape:gk`.
  - **Page:** `/goalkeepers-opta` renders fresh visualizations off the reliable Opta fields — saves leaderboard, save%-vs-shots-faced scatter, clean sheets, distribution accuracy — plus summary cards and a sortable table (`GoalkeeperTableOpta.svelte`). Charts whose data doesn't exist in the Opta feed (distribution-under-pressure, defensive-actions inside/outside) were dropped.
  - **Save% caveat:** `savePct = saves ÷ (saves + goalsConceded)`; the Opta feed omits `goalsConceded` in some matches, so save% is approximate — flagged in a page footnote.
- **Verification:** Cross-checked against FIFA's official Goalkeeping leaderboard (fifa.com, competitionId 285023) via the browser. Same keeper set and broad ordering; several exact matches (Kobel 20, Vozinha 18). Per-keeper save totals differ 0–5 because Opta and FIFA are independent providers — verified match-by-match that the scraper faithfully sums the Opta feed (Gill: 4+6+5+5+3 = 23, matches feed exactly; FIFA's separate feed says 28). `node scripts/scrape-goalkeepers.js --selfcheck` passes; `npm run build` succeeds (both `/goalkeepers` and `/goalkeepers-opta`); `npm run lint` clean.

### Bug Fix — ShotMap penalty box drawn ~1.3m too deep

- **Completed:** 2026-07-14
- **Files changed:** `src/components/match/ShotMap.svelte`
- **What was done:** The static Opta pitch markup drew the 18-yard box edge at SVG `x=136` and the 6-yard box at `x=46.4`, but the scale established by the center-circle radius (`69.717` units, an exact match for 9.15m over the 105m/800px pitch scale) implies the box edges should be at `125.714` (16.5m) and `41.905` (5.5m) — the box was rendered ~10 units (~1.3m) deeper than regulation, mirrored on both sides. This caused borderline shots within that margin (e.g. Norway v England 400021539, shot #11, 109') to render as inside the box while FIFA's official chart correctly shows them on/outside the line; shots comfortably outside both boundaries (e.g. shot #4) already matched. Corrected the box edge coordinates and recomputed the penalty-arc ("D") chord to reconnect to the new box line using the same 9.15m radius (`d=33.714`, `dy=sqrt(r²-d²)=61.023`).
- **Verification:** `npm run build` succeeds (111 pages). Diagnosed via corner-kick raw-coordinate cross-validation and live SVG coordinate extraction against a screenshot of FIFA's official "Attempts at Goal" widget for match 400021539.

### Feature — Teams page (FIFA + Opta sources)

- **Completed:** 2026-07-15
- **Files changed:** `src/lib/data.js` (added `loadTeamStatsConsolidated`, `loadTeamStatsOpta`), `src/components/PlayerStatsTable.svelte` (generalized fixed-column/tab/label props so it can render a team-entity table, not just player), `src/pages/teams.astro` (new), `src/pages/teams-opta.astro` (new), `src/layouts/Base.astro` (nav links)
- **What was done:** Added `/teams` (FIFA-sourced) and `/teams-opta` (Opta-sourced), mirroring the existing player-stats/player-stats-opta split.
  - **FIFA source:** `loadTeamStatsConsolidated` sums each team's per-match `team-stats.json` (48 teams), plus `avgPossession` (mean, not summed) and `goalDifference`.
  - **Opta source:** the Opta feed has no team-totals endpoint, so `loadTeamStatsOpta` sums each team's players' per-match stats from `players-opta.json`. Caught a real data issue while wiring this up: Opta repeats team-outcome fields (`goalsConceded`, `penGoalsConceded`) on every player's row for a match rather than the scorer/keeper only, so naively summing them inflated goals-conceded by ~11x (e.g. France showed 44 conceded instead of 7). Fixed by reading those two fields once per team-per-match (max across that match's rows) instead of summing every row.
  - **Component:** `PlayerStatsTable.svelte` gained `entityKey`/`entityHeader`/`showTeamColumn`/`searchPlaceholder`/`csvPrefix`/`consolidatedTabLabel` props (all defaulted to prior player-page behavior) and now hides the "By Match" tab when no `matchRows` are passed, so both team pages reuse it without touching the two existing player pages.
- **Verification:** `npm run build` succeeds (114 pages, both new routes present with 48 teams each). `npm run lint:fix` clean. Spot-checked aggregates against raw per-match JSON via node (`loadTeamStatsOpta` goals-conceded now matches plausible per-team totals, e.g. Spain 1, Belgium 7, France 7). Loaded both pages on the dev server (200 OK, sortable/searchable table renders, CSV download present) — did not screenshot in a browser.
