...
class e {
  ...
  bezierCurveTo(t, i, h, s, _, e) {
    this._append`C${+t},${+i},${+h},${+s},${(this._x1 = +_)},${(this._y1 = +e)}`;
  }
  arcTo(i, s, _, e, n) {
    ...
    if (null === this._x1) this._append`M${(this._x1 = i)},${(this._y1 = s)}`;
      ...
      if (Math.abs(l * r - o * p) > h && n) {
        ...
        (Math.abs(v - 1) > h && this._append`L${i + v * p},${s + v * l}`,
          this
            ._append`A${n},${n},0,0,${+(l * x > p * d)},${(this._x1 = i + g * r)},${(this._y1 = s + g * o)}`);
      } else this._append`L${(this._x1 = i)},${(this._y1 = s)}`;
    else;
  }
  arc(_, e, n, $, a, r) {
    if (((_ = +_), (e = +e), (r = !!r), (n = +n) < 0)) throw new Error(`negative radius: ${n}`);
    let o = n * Math.cos($),
      p = n * Math.sin($),
      l = _ + o,
      u = e + p,
    ...
    (null === this._x1
      ? this._append`M${l},${u}`
      : (Math.abs(this._x1 - l) > h || Math.abs(this._y1 - u) > h) && this._append`L${l},${u}`,
      n &&
        (d < 0 && (d = (d % i) + i),
        d > s
          ? this
              ._append`A${n},${n},0,1,${x},${_ - o},${e - p}A${n},${n},0,1,${x},${(this._x1 = l)},${(this._y1 = u)}`
          : d > h &&
            this
              ._append`A${n},${n},0,${+(d >= t)},${x},${(this._x1 = _ + n * Math.cos(a))},${(this._y1 = e + n * Math.sin(a))}`));
  }
  rect(t, i, h, s) {
    this
      ._append`M${(this._x0 = this._x1 = +t)},${(this._y0 = this._y1 = +i)}h${(h = +h)}v${+s}h${-h}Z`;
  }
  toString() {
    return this._;
  }
