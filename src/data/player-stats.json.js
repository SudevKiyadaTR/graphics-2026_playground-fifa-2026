...
const INCLUDE_OWN_GOALS_IN_PLAYER_TOTALS = false;
...
function getGoalsByPlayerFromLive(liveData) {
  const byPlayer = {};
  if (!liveData) return byPlayer;

  const goalEvents = [...(liveData.HomeTeam?.Goals || []), ...(liveData.AwayTeam?.Goals || [])];

  for (const goal of goalEvents) {
    const playerId = String(goal?.IdPlayer || "");
    if (!playerId) continue;

    // FIFA goal type 3 is own goal; exclude from player-goal attribution.
    if (!INCLUDE_OWN_GOALS_IN_PLAYER_TOTALS && Number(goal?.Type) === 3) {
      continue;
    }
...
function getPositionRole(positionCode) {
  switch (Number(positionCode)) {
    case 0:
      return "Goalkeeper";
    case 1:
      return "Defender";
  ...
  try {
    ...
    for (const side of ["HomeTeam", "AwayTeam"]) {
      ...
      for (const player of team.Players) {
        ...
        const playerRole = getPositionRole(player.Position);
        ...
        playerData[playerId].goals += goalsByPlayer[playerId] || 0;
        ...
        if (playerData[playerId].position === "Unknown" && playerRole !== "Unknown") {
          playerData[playerId].position = playerRole;
        }
      }
    }
  } catch (error) {
